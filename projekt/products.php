...
if (isset($_POST['delete'])) {
    ...
    if (is_numeric($productId)) {
        ...
        if ($product->delete($productId)) {
            ...
            echo "Nepodarilo sa odstrániť produkt.";
        }
    } else {
        echo "Neplatné ID produktu.";
    }
    exit(); // Ukončite ďalšie spracovanie po odstránení
}
...
?>
    ...
    <script>
        ...
        // Potvrdenie o hromadnom odstránení
        document.getElementById('bulk-delete-form').addEventListener('submit', function (e) {
            if (!confirm("Ste si istí, že chcete odstrániť vybrané produkty?")) {
                e.preventDefault();
            }
        });

        // Odstránenie jednotlivého produktu (voliteľné, ak je potrebné)
        document.querySelectorAll('.delete-product').forEach((icon) => {
            icon.addEventListener('click', function () {
                ...
                if (confirm("Ste si istí, že chcete odstrániť tento produkt?")) {
                    // Odoslanie požiadavky DELETE cez AJAX
                    $.ajax({
                        url: 'products.php',  // Uistite sa, že odkazuje na správnu stránku (túto stránku)
                        type: 'POST',
                        data: {
                            delete: productId,  // Posielame ID produktu na odstránenie
                        },
                        success: function(response) {
                            alert(response);  // Zobraziť odpoveď zo skriptu PHP (úspech alebo chyba)
                            // Voliteľne, znovu načítať stránku alebo odstrániť riadok produktu
                            location.reload();  // Načítať stránku znovu na zobrazenie zmien
                        },
                        error: function() {
